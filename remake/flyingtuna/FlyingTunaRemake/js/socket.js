class WebSocketManager {
  ...
  createConnection(url, callback, filters) {
    ...
    const that = this;
    // ?l= is only understood by tosu >= 2.6 (which injects window.COUNTER_PATH); older builds drop the connection on unknown query strings.
    const query = window.COUNTER_PATH ? `?l=${encodeURI(window.COUNTER_PATH)}` : '';
    this.sockets[url] = new WebSocket(`ws://${this.host}${url}${query}`);
